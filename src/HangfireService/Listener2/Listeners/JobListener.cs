    ...
    public class JobListener : IConsumer<IJobMessage>
    {
        ...
        public async Task Consume(ConsumeContext<IJobMessage> context)
        {
            if (context?.Message != null)
            {
                ...
                _logger.LogInformation("Listener2: Received event {EventName}, job name: {JobName}, service name: {ServiceName}", nameof(IJobMessage), context.Message.JobName, context.Message.ServiceName);
            }
            else
            {
                _logger.LogWarning("Listener2: Received event {EventName} is null.", nameof(IJobMessage));
            }
